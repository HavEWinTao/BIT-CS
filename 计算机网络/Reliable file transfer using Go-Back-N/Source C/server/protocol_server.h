...
#define PF_num 6000
#define MAX_PKT 1024
#define UDPPort_send 10010
#define UDPPort_rec 10011
#define UDPPort_send_1 10013
#define UDPPort_rec_1 10012
...
typedef struct SendThread_para {
	char filename[20];
	char ipaddress[10];
...
void Socket_send(LPVOID lpParam)
{
	FILE* server_log_send;
	server_log_send = fopen("server_log_send.txt", "w+");
	char filename[20];
	char ipaddress[10];
	SendThread_para* sendthread_para = (SendThread_para*)lpParam;
	strcpy(ipaddress,sendthread_para->ipaddress);
	strcpy(filename,sendthread_para->filename);
	time_t now;
	struct tm* tm_now;
	packet pac[PF_num];
	frame frm[PF_num];
	...
	while (1)
	{
		...
		if (pre_turn_head == side_ack_para.side_head && pre_turn_head != 0)
		{
			time(&now);
			tm_now = localtime(&now);
			fprintf(server_log_send,"【%d-%d-%d %d:%d:%d】pdu-to-send=%d,status=TO,ackedNo=%d\n", tm_now->tm_year + 1900, tm_now->tm_mon + 1, tm_now->tm_mday, tm_now->tm_hour, tm_now->tm_min, tm_now->tm_sec, side_ack_para.side_head, side_ack_para.side_head);
			printf( "【%d-%d-%d %d:%d:%d】pdu-to-send=%d,status=TO,ackedNo=%d\n", tm_now->tm_year + 1900, tm_now->tm_mon + 1, tm_now->tm_mday, tm_now->tm_hour, tm_now->tm_min, tm_now->tm_sec, side_ack_para.side_head, side_ack_para.side_head);
			//printf("----------------超时重传%d\n", side_ack_para.side_head);
		}
		int temp_temp = side_ack_para.side_head;
		pre_turn_head = side_ack_para.side_head;
		while (flag < SlideWindow)
		{

			if (temp_temp + flag > packet_num)
				break;
			char* buffer = (char*)&frm[temp_temp + flag];
		...
		{
			TerminateThread(thread_ACK_rec, 0);
			break;
		}

	}
	fclose(server_log_send);
...
void Socket_rec(char* ipaddress)
{
	FILE* server_log_rec;
	server_log_rec = fopen("server_log_rec.txt", "w+");
	time_t now;
	struct tm* tm_now;
	frame rfrm[PF_num];
	...
	while (1)
	{
		...
		if ((Ret = recvfrom(ReceivingSocket, buffer, 2048, 0, (SOCKADDR*)&SenderAddr, &SenderAddrSize)) != SOCKET_ERROR)
		{
			...
			if (temp.seq == frame_num_wanted)
			{
				...
				else
				{
					...
					fprintf(server_log_rec,"【%d-%d-%d %d:%d:%d】pdu_exp=%d,pdu_recv=%d,status=DataErr\n", tm_now->tm_year + 1900, tm_now->tm_mon + 1, tm_now->tm_mday, tm_now->tm_hour, tm_now->tm_min, tm_now->tm_sec, frame_num_wanted, temp.seq);
			...
			else
			{
				time(&now);
				tm_now = localtime(&now);
				fprintf(server_log_rec,"【%d-%d-%d %d:%d:%d】pdu_exp=%d,pdu_recv=%d,status=NoErr\n", tm_now->tm_year + 1900, tm_now->tm_mon + 1, tm_now->tm_mday, tm_now->tm_hour, tm_now->tm_min, tm_now->tm_sec, frame_num_wanted, temp.seq);
		...
		if (rfrm[index - 1].seq == frame_num)
		{
			...
			TerminateThread(Socket_rec_ACK, 0);
			break;
		}
	}
	fclose(server_log_rec);
